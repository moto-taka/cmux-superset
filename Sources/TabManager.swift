...
class TabManager: ObservableObject {
    ...
    func addWorkspace(
        ...
        return withExtendedLifetime((capturedTabs, sourceWorkspace)) {
            ...
            } else if let terminalPanel = newWorkspace.focusedTerminalPanel {
                // For workspaces without explicit working directory, probe the current
                // directory so gitRepoRoot can be set for sidebar repo grouping.
                scheduleInitialWorkspaceGitMetadataRefresh(
                    workspaceId: newWorkspace.id,
                    panelId: terminalPanel.id,
...
#if DEBUG
            dlog("worktree.createFailed error=\(error.localizedDescription)")
#endif
            return nil
        }
    }
